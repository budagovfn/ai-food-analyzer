...
\begin{frame}{Testing}
  ...
  \begin{itemize}\setlength\itemsep{0.3em}
    \item Total coverage \textbf{64\,\%}; \textbf{102} tests, all offline
    \item \texttt{api.py} 90\,\%, \texttt{cli.py} 84\,\%, \texttt{ai\_service.py} 97\,\%; \texttt{nutrition}, \texttt{pipeline}, \texttt{config}, \texttt{models} 100\,\%
    \item The uncovered remainder is the provided \texttt{ai/providers/} adapters
  ...
  \begin{itemize}\setlength\itemsep{0.25em}\small
    ...
    \item Unit audit: the 1975\,kcal payload from Slide 6, pinned wrong before the fix, consistent after
  \end{itemize}
  \end{columns}

  \vspace{0.6em}
  \begin{block}{Honesty note}
    Our concurrency \emph{timing} test is broken: an autouse fixture disables \texttt{time.sleep} globally, so it can't tell parallel from sequential. The \emph{behavioural} guarantee (one failure doesn't break the batch) is still correctly tested.
  \end{block}
\end{frame}
...
\begin{frame}{Limitations \& what we'd do next}
  ...
  \begin{enumerate}\setlength\itemsep{0.3em}
    \item Fix the broken concurrency-timing test (Slide 8).
...
\begin{frame}{Team contributions}
  ...
  \begin{tabularx}{\linewidth}{lXl}
    ...
    \midrule
    \memberone   & Config and storage layers, HTTP API fixes, Docker hardening, unit-audit provider, git/PR workflow, report \& slides & 19/31 ($\sim$61\%) \\
    \membertwo   & AI-service wrapper (retry/timeout/cache), concurrency pipeline, FastAPI endpoint, Dockerfile \& compose & 8/31 ($\sim$26\%) \\
    \memberthree & API and CLI test suites, \texttt{README}, pull-request review and merges & 4/31 ($\sim$13\%) \\
    \bottomrule
  \end{tabularx}\\[0.3em]
  {\scriptsize Counts from \texttt{git shortlog -sn} with the repository's \texttt{.mailmap}, which collapses the several git identities we each committed under.}
  \end{center}
