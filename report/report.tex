...
\texttt{src/api.py} & 0\,\% \\
\texttt{src/cli.py} & 0\,\% \\
\texttt{src/models.py} & 0\,\% \\
\textbf{\texttt{src/} overall} & \textbf{40\,\%} \\
Provided \texttt{ai/} smoke tests & passing \\
\bottomrule
\end{tabular}
\end{center}

Honestly: the lines we have \emph{not} covered are not a deliberate quality choice, they are the entry points we wrote last and have not gotten back to -- \texttt{api.py}, \texttt{cli.py} and the plain-data \texttt{models.py} are all at 0\%, and closing that gap (with \texttt{TestClient} for the API and \texttt{argparse}-level tests for the CLI, both against the same \texttt{FakeVLM}/\texttt{FakeNutrition} fixtures already in \texttt{conftest.py}) is the single highest-priority item left before submission, tracked in \S\ref{sec:limits}. One mock we did have to redesign mid-project: an early version of the \texttt{no\_real\_sleep} autouse fixture in \texttt{tests/test\_pipeline.py}, added to keep the whole suite fast by disabling \texttt{time.sleep} globally, turned out to also silently defeat \texttt{test\_lookup\_all\_runs\_concurrently\_not\_sequentially}'s own use of \texttt{time.sleep(0.3)} to simulate slow I/O -- the test still passes even if the pipeline is made to run strictly sequentially, which we confirmed experimentally and flag as a known, not-yet-fixed issue rather than pretend the concurrency claim in \S\ref{sec:concurrency} is machine-verified end-to-end.
...
\begin{itemize}[itemsep=4pt]
  ...
  \item \textbf{The concurrency test is not actually testing concurrency.} \texttt{test\_lookup\_all\_runs\_concurrently\_not\_sequentially} passes even if \texttt{asyncio.gather} is replaced with a sequential loop, because the file's own \texttt{no\_real\_sleep} autouse fixture disables the \texttt{time.sleep} the test relies on to tell the two cases apart (\S\ref{sec:testing}). The behavioural guarantee itself (Section~6.2(ii)) is still tested correctly; only the \emph{timing} assertion is broken.
